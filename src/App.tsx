...
function App() {
  ...
  const coinDataProvider = useSelector(state => (state as any).coinDataProvider);
  useEffect(() => {
    const coinService: CoinServiceInterface = createCoinService(COIN_DATA_PROVIDER);
    dispatch.coinDataProvider.set(coinService);
  }, []);
  return (
    coinDataProvider ? <Token /> : <>---</>
  )
}
