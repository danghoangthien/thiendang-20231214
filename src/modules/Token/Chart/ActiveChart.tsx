...
const ActiveChart = () => {
  ...
  const activeChartType: number = useSelector(
    (state: any) => state.activeChartType
  );
  ...
  useEffect(() => {
    if (activeCoin) {
      if (activeChartType === 0) {
        coinDataProvider
          .fetchHistoricalData(activeCoin, "usd", activeDayRange)
      ...
      if (activeChartType === 1) {
        coinDataProvider
          .fetchOHLC(activeCoin, activeDayRange)
          .then((data: any) => {
  ...
  return (
    <>
      {activeChartType === 0 && priceChartData && (
        <Price key={activeDayRange} chartData={priceChartData} />
      )}
      {activeChartType === 1 && ohlcChartData && (
        <OHLCChart key={activeDayRange} chartData={ohlcChartData} />
      )}
    </>
  );
};
